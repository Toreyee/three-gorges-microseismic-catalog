...
(@ARGV >= 4 && @ARGV <= 7) || die "Usage: perl $0 PICKS_ROOT STATION_FILE TTIME_FILE REAL_EXECUTABLE [OUTPUT_DIR] [START_DATE=2018-01-01] [NDAY=365]\n";
my ($picks_root,$station,$ttime,$real_executable,$output_dir,$start_date,$nday)=@ARGV;
$picks_root = abs_path($picks_root) // die "Missing PICKS_ROOT\n";
$station = abs_path($station) // die "Missing STATION_FILE\n";
$ttime = abs_path($ttime) // die "Missing TTIME_FILE\n";
$real_executable = abs_path($real_executable) // $real_executable;
$output_dir //= getcwd();
make_path($output_dir) unless -d $output_dir;
$output_dir = abs_path($output_dir);
chdir $output_dir or die "Cannot chdir $output_dir: $!\n";
$start_date //= "2018-01-01";
$nday //= 365;
$start_date =~ /^(\d{4})-(\d{2})-(\d{2})$/ or die "START_DATE must be YYYY-MM-DD\n";
my ($start_year,$start_month,$start_day)=($1+0,$2+0,$3+0);
$picker=1;
...
$year0 = $start_year;
$month0 = $start_month;
$day0 = $start_day - 1; # historical loop increments before processing day 1
...
$numps = $ENV{REAL_BEST_MIN_PICKS} // 30; # minimum number of P and S picks
$gap = $ENV{REAL_BEST_MAX_GAP} // 180; # largest station gap
